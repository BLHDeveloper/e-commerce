"use client";
import React, { useRef } from "react";
import happyCustomers from "./OurHappy.json";

const StarRating = ({ rating }) => {
  const stars = Math.round(rating);
  return (
    <div className="flex gap-0.5">
      {[...Array(5)].map((_, i) => (
        <svg key={i} width="18" height="18" viewBox="0 0 24 24" fill={i < stars ? "#FFC633" : "#E0E0E0"} xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87L18.18 22 12 18.56 5.82 22 7 14.14l-5-4.87 6.91-1.01L12 2z" />
        </svg>
      ))}
    </div>
  );
};

const TestimonialCard = ({ customer }) => (
  <article className="flex-shrink-0 w-[85vw] sm:w-[360px] md:w-[400px] snap-start px-6 sm:px-8 py-6 sm:py-7 rounded-[20px] border border-black/10 flex flex-col gap-3">
    <StarRating rating={customer.rating} />
    <div className="flex items-center gap-1.5">
      <h3 className="text-lg sm:text-xl font-bold text-black leading-snug">{customer.name}</h3>
      <svg width="20" height="20" viewBox="0 0 24 25" fill="none" aria-hidden="true">
        <path d="M12 2.8291C10.0716 2.8291 8.18657 3.40093 6.58319 4.47227C4.97982 5.54362 3.73013 7.06636 2.99218 8.84794C2.25422 10.6295 2.06114 12.5899 2.43735 14.4812C2.81355 16.3725 3.74215 18.1098 5.10571 19.4734C6.46928 20.837 8.20656 21.7656 10.0979 22.1418C11.9892 22.518 13.9496 22.3249 15.7312 21.5869C17.5127 20.849 19.0355 19.5993 20.1068 17.9959C21.1782 16.3925 21.75 14.5075 21.75 12.5791C21.7473 9.99408 20.7192 7.51571 18.8913 5.68783C17.0634 3.85994 14.585 2.83183 12 2.8291ZM16.2806 10.8597L11.0306 16.1097C10.961 16.1795 10.8783 16.2348 10.7872 16.2725C10.6962 16.3103 10.5986 16.3297 10.5 16.3297C10.4014 16.3297 10.3038 16.3103 10.2128 16.2725C10.1218 16.2348 10.039 16.1795 9.96938 16.1097L7.71938 13.8597C7.57865 13.719 7.49959 13.5281 7.49959 13.3291C7.49959 13.1301 7.57865 12.9392 7.71938 12.7985C7.86011 12.6577 8.05098 12.5787 8.25 12.5787C8.44903 12.5787 8.6399 12.6577 8.78063 12.7985L10.5 14.5188L15.2194 9.79848C15.2891 9.72879 15.3718 9.67352 15.4628 9.63581C15.5539 9.59809 15.6515 9.57868 15.75 9.57868C15.8486 9.57868 15.9461 9.59809 16.0372 9.63581C16.1282 9.67352 16.2109 9.72879 16.2806 9.79848C16.3503 9.86816 16.4056 9.95088 16.4433 10.0419C16.481 10.133 16.5004 10.2306 16.5004 10.3291C16.5004 10.4276 16.481 10.5252 16.4433 10.6163C16.4056 10.7073 16.3503 10.79 16.2806 10.8597Z" fill="#01AB31" />
      </svg>
    </div>
    <p className="text-sm sm:text-base text-black/60 leading-relaxed line-clamp-4">
      {customer.description}
    </p>
  </article>
);

const ArrowButton = ({ direction, onClick }) => (
  <button
    onClick={onClick}
    className="p-2 rounded-full hover:bg-gray-100 transition-colors"
    aria-label={direction === "left" ? "Previous testimonials" : "Next testimonials"}
  >
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      {direction === "left" ? (
        <path d="M15 18l-6-6 6-6" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      ) : (
        <path d="M9 18l6-6-6-6" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      )}
    </svg>
  </button>
);

export default function OurHappyCustomers() {
  const scrollRef = useRef(null);

  const scroll = (direction) => {
    if (!scrollRef.current) return;
    const scrollAmount = 420;
    scrollRef.current.scrollBy({
      left: direction === "left" ? -scrollAmount : scrollAmount,
      behavior: "smooth",
    });
  };

  return (
    <section className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      {/* Header */}
      <div className="flex items-end sm:items-center justify-between mb-6 sm:mb-10 gap-4">
        <h2 className="text-2xl sm:text-4xl lg:text-5xl font-bold font-['Integral_CF'] text-black">
          OUR HAPPY CUSTOMERS
        </h2>
        <div className="flex items-center gap-2 flex-shrink-0">
          <ArrowButton direction="left" onClick={() => scroll("left")} />
          <ArrowButton direction="right" onClick={() => scroll("right")} />
        </div>
      </div>

      {/* Carousel */}
      <div
        ref={scrollRef}
        className="flex gap-4 sm:gap-5 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-4"
      >
        {happyCustomers.map((customer) => (
          <TestimonialCard key={customer.id} customer={customer} />
        ))}
      </div>
    </section>
  );
}
